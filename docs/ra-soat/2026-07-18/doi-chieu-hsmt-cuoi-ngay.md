...
## CẬP NHẬT 14:30 (+07) — MỤC 13 ĐÃ ĐÓNG

Đã bổ sung "sửa nội dung phiếu lấy ý kiến ở trạng thái NHÁP" (hành động con cuối cùng còn thiếu của mục 13):
- `src/services/voteService.ts`: `updateDraftVote()` — chỉ phiếu poll + status draft; quyền người tạo/quản lý; chỉ sửa field nội dung (title/description/options/secret/deadline/documentIds/eligibleIds/trackerUserId); không đụng status/ballots; ghi audit.
- `src/ui/pages/PollsPage.tsx`: nút "Sửa" trên thẻ phiếu nháp, tái dùng PollCreateModal chế độ sửa (prefill đầy đủ, kể cả Phiếu kín + Cán bộ theo dõi).
- Hồi quy tự kiểm (a)–(f) PASS: tạo mới/nháp, gửi lấy ý kiến, phiếu kín + redact + signedCount, ký số, filter "Chưa gửi", phiếu đã mở/đóng không hiện nút Sửa (chặn kép UI + service). Guard server đi qua PATCH hợp lệ, không cần sửa backend. Build PASS 2 lần.

**CON SỐ CHỐT MỚI: Web 58/59 ✅ trọn vẹn · 1 🟡 duy nhất = mục 30 (ký số CA thật — hình hài chức năng đã đủ, mô phỏng chờ hợp đồng CA — việc pháp nhân) · 0 ❌.** Mobile nghiệp vụ 37/38 (app native build cần máy ngoài). Chi tiết fix: `reports/fix-muc13.md` (phiên làm việc) — nhật ký trong commit.
