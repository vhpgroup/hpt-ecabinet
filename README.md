...
### Giới hạn HA — ĐÃ GIẢI bằng Redis backplane (GATED)

Hai cơ chế trước đây dùng **state tĩnh trong 1 tiến trình** (per-process): **rate-limit** (`RateLimit.cs`/`ratelimit.js` — đếm theo IP bằng `ConcurrentDictionary`/`Map`) và **WebSocket realtime** (`Ws.cs`/`ws.js` — danh sách client trong RAM tiến trình). Khi chạy nhiều instance App/Web sau cân bằng tải (mô hình HSMT "Web-Server ×2", "App-Server ×2") thì (1) rate-limit đếm rời nhau và (2) client nối instance A không nhận broadcast từ ghi ở instance B.

**Nay ĐÃ CÓ Redis backplane (tự viết, gated qua `REDIS_URL`)** giải cả 2 gap — parity cả bản Node lẫn .NET:

- **Realtime qua Redis Pub/Sub**: mỗi instance SUBSCRIBE kênh `ecabinet:changes`; khi có ghi CRUD, instance đó **chỉ PUBLISH** sự kiện (không gửi trực tiếp cho client local) — mọi instance (kể cả instance phát) nhận lại qua subscribe rồi fanout cho client của mình **đúng 1 lần** → client nối instance nào cũng nhận realtime, **không nhân đôi**.
- **Rate-limit chung**: `hit(key)` khi bật Redis dùng **INCR + PEXPIRE** (đếm chung toàn cụm) → không lách được bằng cách đổi instance.
- **Client RESP tự viết**: nói giao thức Redis TRỰC TIẾP qua `node:net` (Node) / `System.Net.Sockets.TcpClient` (.NET) — **KHÔNG thêm dependency** (đúng triết lý đã tự viết JWT/SigV4/WS RFC6455). 2 kết nối: 1 dành riêng SUBSCRIBE blocking, 1 cho PUBLISH + lệnh rate-limit.
- **Fallback an toàn**: Redis rớt → WS tự về fanout local-only + tự kết nối lại lũy tiến; rate-limit **fail-open** (cho qua, không chặn toàn hệ); PING keepalive. Lỗi Redis **không làm sập request nghiệp vụ**.

**GATED — tương thích ngược tuyệt đối**: **không đặt `REDIS_URL`** → hành vi Y HỆT hiện tại (broadcast local, rate-limit in-RAM); demo/dev/1-node không đổi. Đặt `REDIS_URL=redis://[:pass@]host:port[/db]` (`rediss://` cho TLS) → bật backplane. Hạ tầng: service `redis` trong `docker-compose*.yml` gated qua **profile `scale`** (mặc định không tạo). Hướng dẫn bật + kiểm chứng 2 instance đồng bộ: xem `docs/HUONG-DAN-TRIEN-KHAI-VA-HSMT.md` **mục A3.2** và báo cáo `docs/ra-soat/2026-07-21/redis-backplane.md`.

Refresh-token/session (`Sessions.cs`/`sessions.js`) vốn **không có gap này** (đã lưu qua CSDL).

> **Kiểm chứng**: lớp giao thức RESP (encode/parse) test bằng test-vector byte; backplane + rate-limit test bằng **fake Redis in-process** (giả pub/sub + INCR/EXPIRE) — 2 "instance" giả publish qua lại + đếm chung 1 key (Node smoke nhóm `12-REDIS-BACKPLANE`, .NET nhóm `12-REDIS`). Kiểm chứng với **Redis thật** (2 instance API sau LB) do chủ dự án chạy theo hướng dẫn A3.2 (sandbox chặn socket nên chưa chạy Redis thật ở đây).
