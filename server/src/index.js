...
import { attachRealtime, fanoutLocal, clientCount } from './ws.js';
import { initBackplane, backplane } from './redis.js';
 ...
 *
 * SCALE NGANG (App×2 sau LB): nếu Redis backplane BẬT -> CHỈ PUBLISH lên kênh chung
 * `ecabinet:changes` (KHÔNG gửi local trực tiếp) — mọi instance (kể cả instance phát)
 * SUBSCRIBE kênh này, nhận message rồi fanoutLocal() cho client của mình ĐÚNG 1 LẦN
 * => chống double-send + client nối instance khác vẫn nhận (xem server/src/redis.js).
 * Nếu Redis TẮT hoặc publish LỖI (mất kết nối) -> fallback fanoutLocal() như cũ.
 * Fire-and-forget: không await (poke-then-pull chịu được trễ; không chặn request ghi).
 */
const notifyChange = (collection, action, id) => {
  const event = { type: 'change', collection, action, id, at: new Date().toISOString() };
  const bp = backplane();
  if (bp && bp.up) {
    // publishChange trả false nếu Redis rớt ngay lúc publish -> fallback local.
    bp.publishChange(event)
      .then((published) => { if (!published) fanoutLocal(event); })
      .catch(() => fanoutLocal(event)); // phòng lỗi bất ngờ -> vẫn phát local (không mất realtime)
  } else {
    fanoutLocal(event); // đường cũ (1 instance / Redis tắt) — hành vi Y HỆT
  }
};

/**
 * RATE-LIMIT hợp nhất: Redis BẬT -> đếm CHUNG toàn cụm (INCR+PEXPIRE, xem redis.js);
 * Redis rớt giữa chừng -> backplane trả null -> FAIL-OPEN (cho qua, không chặn toàn hệ)
 * + log trong backplane. Redis TẮT (mặc định) -> Map in-RAM như cũ (hit()) — hành vi Y HỆT.
 * Trả cùng shape { ok, remaining, retryAfterSec }.
 */
async function rateHit(key, max, windowMs) {
  const bp = backplane();
  if (bp && bp.up) {
    const r = await bp.rateHit(key, max, windowMs);
    if (r) return r;
    // Redis vừa rớt -> FAIL-OPEN: coi như qua (không tụt về Map để tránh đếm lệch nửa vời).
    return { ok: true, remaining: max, retryAfterSec: 1 };
  }
  return hit(key, max, windowMs); // đường cũ (1 instance / Redis tắt)
}
...
app.add('POST', '/api/auth/login', async (req, res) => {
  ...
  // GĐ4: chống brute-force — giới hạn theo IP + tài khoản (dùng chung qua Redis khi bật)
  const rl = await rateHit(`login:${clientIp(req)}:${username}`, LOGIN_MAX, LOGIN_WINDOW);
  if (!rl.ok) {
...
initDb()
  .then(async () => {
    ...
    // chung -> fanoutLocal cho client WS của tiến trình này. Không REDIS_URL -> trả null,
    // KHÔNG mở kết nối, mọi thứ chạy local như cũ (tương thích ngược tuyệt đối).
    await initBackplane(fanoutLocal);

    const server = http.createServer((req, res) => {
      // GĐ4: rate-limit toàn cục theo IP (trừ health check). Async vì Redis (khi bật) đếm chung.
      if (req.url !== '/health') {
        rateHit('ip:' + clientIp(req), RATE_MAX, RATE_WINDOW).then((rl) => {
          if (!rl.ok) {
            res.writeHead(429, {
          ...
          app.handle(req, res);
        }).catch(() => app.handle(req, res)); // lỗi bất ngờ -> fail-open, không chặn nghiệp vụ
        return;
      }
      app.handle(req, res);
    });
    ...
    server.listen(PORT, () => {
      ...
      const bp = backplane();
      console.log(bp
        ? `[server] Redis backplane: BẬT (App×N sau LB — realtime + rate-limit đồng bộ toàn cụm)`
        : `[server] Redis backplane: TẮT (1 instance — đặt REDIS_URL để chạy App×2 sau cân bằng tải)`);
    });
  })
  .catch((e) => {
